 ...
 **Explanation:** The intersected node's value is `2` (note that this must not be `0` if the two lists intersect). From the head of **A**, it reads as `[1,9,1,2,4]`. From the head of **B**, it reads as `[3,2,4]`. There are `3` nodes before the intersected node in **A**; There is `1` node before the intersected node in **B**.
 ...
 **Explanation:** From the head of **A**, it reads as `[2,6,4]`. From the head of **B**, it reads as `[1,5]`. Since the two lists do not intersect, **intersectVal** must be `0`, while **skipA** and **skipB** can be arbitrary values. The two lists do not intersect, so return **null**.
 ...
 ## Follow up:
 Could you write a solution that runs in **O(m + n)** time and use only **O(1)** memory (i.e., constant space)?
 */
